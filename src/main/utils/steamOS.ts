...
const layoutId = "3080264545"; // VCord Layout v2
...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} VCord Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} VCord's recommended Steam Deck controller settings?
